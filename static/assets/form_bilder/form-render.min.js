...
$(document).ready(function () {
    ...
    $(document).on("change", "input[type='file'].UploadFileAjax", function () {
        ...
        $.ajax({
            ...
            data: formData,
            processData: false,
            contentType: false, error: function (data) {
                // // console.log(data.zresponseJSON);
            },
            success: function (data) {
                ...
                $(dis).closest("div").prev("input.FileUpload").val(data)
                $(dis).closest(".UploadFileAjax-div").remove();
            }
        });
    });
    */
...
function showforms(formData, prefix = 'extData', datas = []) {
    if (!formData) return

    var json = {}
    JSON.parse(formData).forEach((val) => {
        // val.name = prefix+'[' + val.name + ']'
        val.name = prefix + '_' + val.name
        json[val.name] = val
    })

    if (Object.values(datas).length) {
        for (key in datas) {
            key_ = prefix + "_" + key.replace("[]", "")
            if (json[key_]) {
                if (["select", "radio-group", "checkbox-group"].includes(json[key_].type)) {
                    json[key_].values.forEach((v, k) => {
                        json[key_].values[k]['selected'] = false
                        if (datas[key].includes(v.value)) json[key_].values[k]['selected'] = true
                    })
                } else {
                    json[key_].value = datas[key][0]
                }
            }
        }
    }

    formData = JSON.stringify(Object.values(json))
