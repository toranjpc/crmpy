function JSONedtr(data, outputElement, config = {}) {
	...
	JSONedtr.level = function (node, lvl = 0) {
		...
		$.each(node, function (key, value) {
			...
			if (typeof value == 'object') {
				...
				output += '<div class="jse--row jse--row--array" id="jse--row-' + JSONedtr.i + '"><input type="text" readonly class="form-control col-10 jse--key jse--array" data-level="' + lvl + '" value="' + key + '"> ';
				output += JSONedtr.level(value, lvl + 1);
				output += '<div class="jse--delete btn btn-danger d-none">✖</div></div>';
			} else {
				if (typeof value == 'string')
					value = value.replace(/\"/g, "&quot;");
				output += '<div class="jse--row" id="jse--row-' + JSONedtr.i + '"><input type="text" readonly class="form-control col-10 jse--key" data-level="' + lvl + '" value="' + key + '"> <input type="text" class="form-control col-10 jse--value" value="' + value + '" data-key="' + key + '"><div class="jse--delete btn btn-danger d-none">✖</div></div>';
			}
		})

		output += '<div class="jse--row jse--add" data-level="' + lvl + '"><button class="btn btn-info jse--plus">✚</button></div>';
	...
	JSONedtr.getData = function (node = $(JSONedtr.outputElement + ' > .jse--row > input')) {
		...
		$.each(node, function () {
			...
			if ($(this).hasClass('jse--value')) {
				result[$(this).closest(".jse--row").find('.jse--key').val()] = $(this).val();
			}
	...
	JSONedtr.addRowForm = function (plus) {
		...
		var typeofHTML = '<select class="jse--typeof form-control col-10">' +
			'<option value="text" selected="selected">Text, Number</option>' +
			'<option value="object">Object, Array</option>' +
			// '<option value="boolean">Boolean</option>' +
			'</select>';

		$(plus).html('<input type="text" class="form-control col-10 jse--key" data-level="' + lvl + '" value=""> <span class="jse--typeof"> ' + typeofHTML + ' </span><input type="text" class="form-control col-10 jse--value jse--value__new" value=""><button class="btn btn-info  jse--save">Save</button><button class="btn btn-danger jse--cancel">Cancel</button>');
		$(plus).children('.jse--key').focus();

		$(plus).find('select.jse--typeof').change(function () {
			switch ($(this).val()) {
				case 'text':
					$(this).parent().siblings('.jse--value__new').replaceWith('<input type="text" class="form-control col-10 jse--value jse--value__new" value="">');
		...
		$('.jse--row.jse--add .jse--cancel').click(function (e) {
			...
			$(e.currentTarget.parentElement).html('<button class="btn btn-info jse--plus">✚</button>');
	...
	JSONedtr.addRow = function (row) {
		...
		switch (typeOf) {
			...
			case 'boolean':
				if ($(row).find('.jse--value__new').is(':checked')) {
					$(row).find('.jse--value__new').replaceWith('<input type="text" class="form-control col-10 jse--value" value="true" data-key="' + key + '">');
				} else {
					$(row).find('.jse--value__new').replaceWith('<input type="text" class="form-control col-10 jse--value" value="false" data-key="' + key + '">');
				}
				break;
			case 'object':
				$(row).find('.jse--key').addClass('jse--object');
				$(row).append('<div class="xxx jse--row jse--add" data-level="' + (lvl + 1) + '"><button class="btn btn-info jse--plus">✚</button></div>');
				$(row).addClass('jse--row-object');
				break;
		}

		$(row).append('<div class="jse--delete btn btn-danger d-none">✖</div>');
		...
		$(row).after('<div class="jse--row jse--add" data-level="' + lvl + '"><button class="btn btn-info jse--plus">✚</button></div>');
...
// $(document).on("mouseover", ".jse--row", function () {
// 	$(".jse--delete").addClass("d-none");
// 	$(this).find(".jse--delete").removeClass("d-none");
// });